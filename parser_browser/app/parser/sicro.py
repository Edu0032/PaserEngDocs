...
RE_SICRO_A = re.compile(
    rf"^Insumo\s+(?P<code>E\s*\d+)\s+(?P<bank>SICRO\s*3?|DNIT)\s+(?P<desc>.+?)\s+(?P<quant>{_NUM})\s+(?P<util_op>{_NUM})\s+(?P<util_imp>{_NUM})\s+(?P<custo_op>{_NUM})\s+(?P<custo_imp>{_NUM})\s+(?P<custo_hor>{_NUM})$",
    re.IGNORECASE,
)
RE_SICRO_B = re.compile(
    rf"^Insumo\s+(?P<code>P\s*\d+)\s+(?P<bank>SICRO\s*3?|DNIT)\s+(?P<desc>.+?)\s+(?P<quant>{_NUM})\s+(?P<salario_hora>{_NUM})\s+(?P<custo_hor>{_NUM})$",
    re.IGNORECASE,
)
RE_SICRO_C = re.compile(
    rf"^Insumo\s+(?P<bank>SICRO\s*3?|DNIT)\s+(?P<code>M\s*\d+)\s+(?P<desc>.+?)\s+(?P<quant>{_NUM})\s+(?P<und>[A-Za-z0-9/%²³]+)\s+(?P<preco_unit>{_NUM})\s+(?P<custo_hor>{_NUM})$",
    re.IGNORECASE,
)
...
RE_SICRO_C_SIMPLE = re.compile(
    rf"^Insumo\s+(?P<bank>SICRO\s*3?|DNIT)\s+(?P<code>M\s*\d+)\s+(?P<desc>.+?)\s+(?P<quant>{_NUM})\s+(?P<und>[A-Za-z0-9/%²³]+)\s+(?P<custo_hor>{_NUM})$",
    re.IGNORECASE,
)
RE_SICRO_E = re.compile(
    rf"^Tempo\s+Fixo\s+(?P<bank>SICRO\s*3?|DNIT)\s+(?P<insumo>M\s*\d+)\s+(?P<desc>.+?)\s+(?P<code>\d+)\s+(?P<quant>{_NUM})\s+(?P<und>[A-Za-z0-9/%²³]+)\s+(?P<preco_unit>{_NUM})\s+(?P<custo_hor>{_NUM})$",
    re.IGNORECASE,
)
RE_SICRO_E_SIMPLE = re.compile(
    rf"^Tempo\s+Fixo\s+(?P<bank>SICRO\s*3?|DNIT)\s+(?P<insumo>M\s*\d+)\s+(?P<desc>.+?)\s+(?P<code>\d+)\s+(?P<quant>{_NUM})\s+(?P<und>[A-Za-z0-9/%²³]+)\s+(?P<custo_hor>{_NUM})$",
    re.IGNORECASE,
)
RE_SICRO_F_BASE = re.compile(
    rf"^(?:(?:Momento\s+de\s+Transporte)\s+)?(?P<bank>SICRO\s*3?|DNIT)\s+(?P<insumo>M\s*\d+)\s+(?P<desc>.+?)\s+(?P<quant>{_NUM})\s+(?P<und>[A-Za-z0-9/%²³]+)\s+(?P<code_ln>\d+)$",
    re.IGNORECASE,
)
...
class SicroBlockData:
    ...
    paginas: set[int] = field(default_factory=set)
...
def _normalize_code(code: str) -> str:
    text = re.sub(r"\s+", " ", str(code or "").replace("\xa0", " ").strip())
    # SICRO families often appear as "M 0192", "E 9010" or "P 9821"
    # when text is split by the PDF extractor. In SICRO context these are single codes.
    m = re.fullmatch(r"([EMPemp])\s+(\d{3,6})", text)
    if m:
...
def _parse_section_line(section: str, line: str) -> tuple[Optional[LinhaComposicao], Optional[str]]:
    ...
    if section == "C":
        ...
        simple = False
        if not m:
            m = RE_SICRO_C_SIMPLE.match(line)
            simple = bool(m)
        if not m:
            return None, None
        quant = _parse_num(m.group("quant"))
        custo_hor = _parse_num(m.group("custo_hor"))
        preco_raw = None if simple else m.group("preco_unit")
        preco_unit = _parse_num(preco_raw) if preco_raw is not None else None
        code, bank, evidence = identify_code_and_bank(m.group("bank"), m.group("code"))
        det = _with_numeric_source(_section_details("C", preco_unitario=preco_unit, custo_horario=custo_hor, code_bank_detection=evidence), quant=m.group("quant"), valor_unit=preco_raw, total=m.group("custo_hor"), preco_unitario=preco_raw, custo_horario=m.group("custo_hor"))
        line_obj = _make_line(
    ...
    if section == "E":
        ...
        simple = False
        if not m:
            m = RE_SICRO_E_SIMPLE.match(line)
            simple = bool(m)
        if not m:
            return None, None
        quant = _parse_num(m.group("quant"))
        ...
        det = _with_numeric_source(_section_details(
            ...
            custo_horario=custo_hor,
        ), quant=m.group("quant"), valor_unit=preco_raw, total=m.group("custo_hor"), preco_unitario=preco_raw, custo_horario=m.group("custo_hor"))
        line_obj = _make_line(
...
def _split_merged_sicro_rows(line: str, section: str) -> List[str]:
    """Split text lines where the PDF extractor glued multiple SICRO rows together.

    This is section-aware: every SICRO section has its own row marker. We split only on
    repeated body-row markers, never on values inside a description. This prevents cases
    such as section C material rows being swallowed into one large description.
    """
    raw = _clean(line)
    if not raw:
        return []
    patterns = {
        "A": r"Insumo\s+E\s*\d{3,5}\s+SICRO\s*3?\b",
        "B": r"Insumo\s+P\s*\d{3,5}\s+SICRO\s*3?\b",
        "C": r"Insumo\s+SICRO\s*3?\s+M\s*\d{3,5}\b",
        "D": r"(?:Atividade\s+Auxiliar|Composi(?:ç[aã]o|cao)\s+Auxiliar|Auxiliar)\s+SICRO\s*3?\s+\d{7}\b",
        "E": r"Tempo\s+Fixo\s+SICRO\s*3?\s+M\s*\d{3,5}\b",
        "F": r"(?:Momento\s+de\s+Transporte\s+)?SICRO\s*3?\s+M\s*\d{3,5}\b",
    }
    pat = patterns.get(section or "")
    if not pat:
        return [raw]
    ...
    for idx, match in enumerate(matches):
        ...
        piece = _clean(raw[start:end])
        if piece:
            pieces.append(piece)
    prefix = _clean(raw[: matches[0].start()])
    if prefix and not any(prefix.upper().startswith(x) for x in ("A ", "B ", "C ", "D ", "E ", "F ")):
...
def _consume_section_f(lines: List[str], start_idx: int) -> Tuple[List[LinhaComposicao], Dict[str, Any], int]:
    ...
    while idx < len(lines):
        ...
        if branch_pos < len(branch_order):
            ...
            if "quantidade_dmt" not in branch:
                ...
                if qtd is not None:
                    ...
                    branch["quantidade_dmt_text"] = clean_numeric_text(raw)
                    idx += 1
                    continue
            if "preco_unitario_dmt" not in branch:
                ...
                if price is not None:
                    ...
                    m_price_text = re.search(_PRICE, _clean(raw), flags=re.IGNORECASE)
                    if m_price_text:
                        branch["preco_unitario_dmt_text"] = clean_numeric_text(m_price_text.group("value"))
        ...
        if maybe_cost is not None:
            ...
            final_cost_text = clean_numeric_text(raw)
            idx += 1
            break
        idx += 1
    ...
    ), quant=m.group("quant"), total=final_cost_text if final_cost_text else (str(final_cost).replace(".", ",") if final_cost is not None else ""), custo_horario=final_cost_text if final_cost_text else "")
    row = _make_line(
        code=insumo,
        bank=bank,
        desc=desc,
...
def extract_sicro_blocks_from_text(
    *,
    ...
    def ensure_block(principal: LinhaComposicao, fallback_item: str, page_no: int) -> SicroBlockData:
        ...
        block.paginas.add(page_no)
        if block.principal is not None:
            try:
                block.principal.detalhes = dict(block.principal.detalhes or {})
                block.principal.detalhes.setdefault("page", page_no)
            except Exception:
                pass
        return block
        ...
        while i < len(lines):
            ...
            if m_comp:
                ...
                if principal is not None and _canon_bank(principal.banco) == "SICRO":
                    current = ensure_block(principal, pending_item, page_no)
                    current_section = ""
            ...
            if not current_section:
                if re.match(r"^Insumo\s+E\s*\d+\s+SICRO", line, flags=re.IGNORECASE):
                    current_section = "A"
                elif re.match(r"^Insumo\s+P\s*\d+\s+SICRO", line, flags=re.IGNORECASE):
                    current_section = "B"
                elif re.match(r"^Insumo\s+SICRO\s*3?\s+M\s*\d+", line, flags=re.IGNORECASE):
                    current_section = "C"
                elif re.match(r"^(Atividade\s+Auxiliar|Composi(?:ç[aã]o|cao)\s+Auxiliar|Auxiliar)\s+SICRO", line, flags=re.IGNORECASE):
                    current_section = "D"
                elif re.match(r"^(Momento\s+de|Transporte\s+SICRO|SICRO\s*3?\s*M\s*\d+)", line, flags=re.IGNORECASE):
                    current_section = "F"
                if current_section:
            ...
            split_rows = _split_merged_sicro_rows(line, current_section) if current_section else [line]
            if len(split_rows) > 1:
                lines[i:i+1] = split_rows
                line = lines[i]

            if current_section == "F" and (line.upper().startswith("MOMENTO DE") or re.match(r"^(?:Transporte\s+)?SICRO\s*3?\s+M\s*\d+", line, flags=re.IGNORECASE)):
                aux_rows, f_row_dict, consumed = _consume_section_f(lines, i)
                if consumed:
                    current.paginas.add(page_no)
                    if isinstance(f_row_dict, dict):
                        f_row_dict.setdefault("detalhes", {})["page"] = page_no
                    current.detalhes["sicro"]["secoes"]["F"].append(f_row_dict)
                    # Não duplicamos ramos de DMT em composicoes_auxiliares; a seção F
                    # possui estrutura própria em detalhes.sicro.secoes.F.
                    i += consumed
                    continue
            ...
            if parsed is not None:
                current.paginas.add(page_no)
                if not isinstance(parsed.detalhes, dict):
                    parsed.detalhes = {}
                parsed.detalhes["page"] = page_no
            ...
            before_summary = dict(current.detalhes.get("sicro", {}).get("resumos", {}) or {})
            _capture_summary(current, line)
            after_summary = dict(current.detalhes.get("sicro", {}).get("resumos", {}) or {})
...
def _source_text(row: Dict[str, Any], field: str, fallback_field: str | None = None) -> Any:
    det = row.get("detalhes") if isinstance(row.get("detalhes"), dict) else {}
    src = det.get("numeric_source") if isinstance(det, dict) else None
    if isinstance(src, dict):
        ...
        if isinstance(meta, dict) and meta.get("source_text") not in (None, ""):
            return clean_numeric_text(meta.get("source_text"))
    key = fallback_field or field
    value = row.get(key)
    if isinstance(value, float):
        # Avoid exporting Python/JSON floats as final SICRO values when no PDF token exists.
...
def _clean_empty(obj: Any) -> Any:
    if isinstance(obj, dict):
        cleaned = {k: _clean_empty(v) for k, v in obj.items() if v not in (None, "", [], {})}
        return cleaned
    if isinstance(obj, list):
        return [_clean_empty(v) for v in obj if v not in (None, "", [], {})]
    return obj


def _sicro_public_row(sec: str, row: Dict[str, Any]) -> Dict[str, Any]:
    """Return a clean, section-specific SICRO row.

    Public SICRO rows intentionally do not reuse SINAPI-like fields such as
    ``descricao`` or ``valor_unit`` when the PDF column has a domain-specific name
    such as ``Equipamentos`` or ``Preço Unitário``. This keeps the JSON readable and
    faithful to the SICRO table.
    """
    row = apply_numeric_sources_to_row(dict(row or {}))
    det = row.get("detalhes") if isinstance(row.get("detalhes"), dict) else {}
    base_bank = row.get("banco") or row.get("banco_coluna") or "SICRO"
    if sec == "A":
        return _clean_empty({
            "codigo": row.get("codigo"),
            "banco": base_bank,
            "equipamento": row.get("descricao"),
            "quantidade": _source_text(row, "quant"),
            "utilizacao": {
                "operativa": _source_text({"detalhes": {"numeric_source": det.get("numeric_source", {})}}, "utilizacao_operativa") or det.get("utilizacao_operativa"),
                "improdutiva": _source_text({"detalhes": {"numeric_source": det.get("numeric_source", {})}}, "utilizacao_improdutiva") or det.get("utilizacao_improdutiva"),
            },
            "custo_operacional": {
                "operativa": _source_text({"detalhes": {"numeric_source": det.get("numeric_source", {})}}, "custo_operacional_operativa") or det.get("custo_operacional_operativa"),
                "improdutiva": _source_text({"detalhes": {"numeric_source": det.get("numeric_source", {})}}, "custo_operacional_improdutiva") or det.get("custo_operacional_improdutiva"),
            },
            "custo_horario": _source_text({"detalhes": {"numeric_source": det.get("numeric_source", {})}}, "custo_horario") or _source_text(row, "total"),
        })
    if sec == "B":
        return _clean_empty({
            "codigo": row.get("codigo"),
            "banco": base_bank,
            "mao_obra": row.get("descricao"),
            "quantidade": _source_text(row, "quant"),
            "salario_hora": _source_text({"detalhes": {"numeric_source": det.get("numeric_source", {})}}, "salario_hora") or _source_text(row, "valor_unit"),
            "custo_horario": _source_text({"detalhes": {"numeric_source": det.get("numeric_source", {})}}, "custo_horario") or _source_text(row, "total"),
        })
    if sec == "C":
        return _clean_empty({
            "banco": base_bank,
            "codigo": row.get("codigo"),
            "material": row.get("descricao"),
            "quantidade": _source_text(row, "quant"),
            "unidade": row.get("und"),
            "preco_unitario": _source_text({"detalhes": {"numeric_source": det.get("numeric_source", {})}}, "preco_unitario") or _source_text(row, "valor_unit"),
            "custo_horario": _source_text({"detalhes": {"numeric_source": det.get("numeric_source", {})}}, "custo_horario") or _source_text(row, "total"),
        })
    if sec == "D":
        return _clean_empty({
            "banco": base_bank,
            "codigo": row.get("codigo"),
            "atividade_auxiliar": row.get("descricao"),
            "quantidade": _source_text(row, "quant"),
            "unidade": row.get("und"),
            "preco_unitario": _source_text({"detalhes": {"numeric_source": det.get("numeric_source", {})}}, "preco_unitario") or _source_text(row, "valor_unit"),
            "custo_horario": _source_text({"detalhes": {"numeric_source": det.get("numeric_source", {})}}, "custo_horario") or _source_text(row, "total"),
    ...
    if sec == "E":
        return _clean_empty({
            "banco": base_bank,
            "insumo": det.get("insumo_origem") or row.get("codigo"),
            "tempo_fixo": row.get("descricao"),
            "codigo": det.get("codigo_servico") or row.get("codigo"),
            "quantidade": _source_text(row, "quant"),
            "unidade": row.get("und"),
            "preco_unitario": _source_text({"detalhes": {"numeric_source": det.get("numeric_source", {})}}, "preco_unitario") or _source_text(row, "valor_unit"),
            "custo_horario": _source_text({"detalhes": {"numeric_source": det.get("numeric_source", {})}}, "custo_horario") or _source_text(row, "total"),
    ...
    if sec == "F":
        raw_dmt = det.get("dmt") if isinstance(det.get("dmt"), dict) else {}
        dmt: Dict[str, Any] = {}
        for branch_name, branch in raw_dmt.items():
            if not isinstance(branch, dict):
                continue
            dmt[branch_name] = _clean_empty({
        ...
        return _clean_empty({
            "banco": base_bank,
            "insumo": det.get("insumo_origem") or row.get("codigo"),
            "momento_transporte": row.get("descricao"),
            "quantidade": _source_text(row, "quant"),
            "unidade": row.get("und"),
            "dmt": dmt,
            "custo_horario": _source_text({"detalhes": {"numeric_source": det.get("numeric_source", {})}}, "custo_horario") or _source_text(row, "total"),
        })
    return _clean_empty(row)


def _sanitize_sicro_materialized_payload(sicro: Dict[str, Any]) -> Dict[str, Any]:
    """Materialize SICRO directly as domain sections.

    This is the internal handoff to the final exporter. It intentionally avoids the
    old ``detalhes.sicro.secoes.A`` staging shape in the public result.
    """
    secoes = (sicro or {}).get("secoes") if isinstance(sicro, dict) else {}
    ...
    if isinstance(secoes, dict):
        for sec in ("A", "B", "C", "D", "E", "F"):
            rows = secoes.get(sec) or []
            if not isinstance(rows, list):
                continue
            cleaned = [_sicro_public_row(sec, row) for row in rows if isinstance(row, dict)]
            if cleaned:
                public_sections[aliases[sec]] = cleaned
    resumos = (sicro or {}).get("resumos") if isinstance(sicro, dict) else None
    if isinstance(resumos, dict) and resumos:
        public_sections["resumos"] = _clean_empty(resumos)
    return _clean_empty(public_sections)
...
def materialize_sicro_block(block: SicroBlockData) -> BlocoComposicao:
    ...
    try:
        staging = (block.detalhes or {}).get("sicro", {}).get("secoes", {}) if isinstance(block.detalhes, dict) else {}
        if isinstance(staging, dict):
            for rows in staging.values():
                if isinstance(rows, list):
                    for row in rows:
                        det = row.get("detalhes") if isinstance(row, dict) else None
                        pg = det.get("page") if isinstance(det, dict) else None
                        if isinstance(pg, int):
                            section_pages.add(pg)
    except Exception:
        section_pages = set()
    principal_page = None
    try:
        principal_page = (block.principal.detalhes or {}).get("page") if block.principal is not None else None
    except Exception:
        principal_page = None
    paginas_set: set[int] = set(section_pages)
    if not paginas_set and principal_page:
        paginas_set.add(int(principal_page))
    elif principal_page and paginas_set and abs(min(paginas_set) - int(principal_page)) <= 1:
        paginas_set.add(int(principal_page))
    paginas = sorted(int(p) for p in paginas_set if p)
    # SICRO é organizado por seções próprias. Não duplicamos linhas em insumos/auxiliares
    # SINAPI-like, pois isso polui e torna difícil conferir com o PDF.
