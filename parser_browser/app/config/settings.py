...
def get_settings(defaults: dict | None = None) -> AppSettings:
    ...
    if is_prod and origins == ['*']:
        origins = []
    return AppSettings(
        environment=environment,
        docs_enabled=_bool_env('API_PDF_DOCS_ENABLED', docs_default),
        api_key=os.getenv(auth_defaults.get('env_var_name', 'API_PDF_API_KEY'), ''),
        ...
        docling_timeout_seconds=_int_env('API_PDF_DOCLING_TIMEOUT_SECONDS', int(timeout_defaults.get('docling_timeout_seconds', 120))),
    )
