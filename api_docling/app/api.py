...
async def _execute_docling_structure_extraction(pdf_bytes: bytes, request: ParseDocumentRequestModel, client: DoclingClient, *, timeout_seconds: int) -> JSONResponse:
    ...
    if not bypass_cache:
        max_entries = int(getattr(client, '_api_structure_cache_max_entries', 32) or 32)
        cache[cache_key] = json.loads(json.dumps(clean_payload))
...
def create_app() -> FastAPI:
    ...
    setattr(app.state.docling_client, '_api_structure_cache_max_entries', settings.docling_cache_max_entries)
    app.add_middleware(
        CORSMiddleware,
    ...
    async def preflight_fallback(full_path: str) -> Response:
        # CORSMiddleware handles browser preflight when Origin is present.
        # This fallback keeps plain OPTIONS checks from failing on Render/Lovable proxies.
    ...
    def version() -> Dict[str, Any]:
        ...
        return {
            ...
            'security': {
                ...
                'cors_allow_methods': list(settings.cors_allow_methods),
                'cors_allow_headers': list(settings.cors_allow_headers),
                'docling_timeout_seconds': settings.docling_timeout_seconds,
                'cache_max_entries': settings.docling_cache_max_entries,
            },
        }
