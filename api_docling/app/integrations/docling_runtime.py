...
def _build_pipeline_options(PdfPipelineOptions: Any, cfg: EmbeddedDoclingConfig) -> Any:
...
def _get_cached_converter(
    *,
    ...
    """Return a cached Docling converter for repeated Render requests.

    The seed PDF changes on each request, but the converter/pipeline config usually does not.
    Reusing it avoids paying converter initialization on every extraction and keeps quality identical.
    """
    key = (bool(cfg.do_cell_matching), str(cfg.table_mode or 'fast').strip().lower(), bool(cfg.do_ocr))
    with _DOCLING_CONVERTER_LOCK:
        converter = _DOCLING_CONVERTER_CACHE.get(key)
        if converter is not None:
            return converter, True, 0.0
        converter_start = time.perf_counter()
        pipeline_options = _build_pipeline_options(PdfPipelineOptions, cfg)
        converter = DocumentConverter(
            format_options={
                InputFormat.PDF: PdfFormatOption(pipeline_options=pipeline_options),
            }
        )
        init_ms = round((time.perf_counter() - converter_start) * 1000.0, 2)
        _DOCLING_CONVERTER_CACHE[key] = converter
        return converter, False, init_ms
...
def extract_structures_with_embedded_docling(
    ...
    *,
    ...
    converter, converter_cache_hit, converter_init_ms = _get_cached_converter(
    ...
    timing['converter_init_ms'] = converter_init_ms
    timing['converter_cache_hit'] = converter_cache_hit
